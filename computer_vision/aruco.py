...
def get_rt_arucos(frame, marker_size, camera_matrix, dist_coeffs):
    ...
    if ids is not None and (4 in ids) and (5 in ids):
        ...
        for (marker_corner, marker_id) in zip(corners, ids):
            ...
            if marker_id==5:
                aruco_markers[marker_id] = [center_x,center_y,treated_corners]
            
            i+=1
    
        return aruco_markers
    elif P_VISION:
        if not AT_ROBOT in ids: print("ERROR: Could not detect Thymio")
        if not AT_DESTINATION in ids: print("ERROR: Could not detect Destination")
    return {}
